...
共 **151** 個參數，全部可在 Cocos 的 Inspector 調整。
...
| `autoCountMax` | `99` | 「自訂」局數的上限。預設 99 = 只讓玩家輸入兩位數；打到會超過就不再吃輸入 |
| `stopAmountMax` | `999999` | 停止條件金額的上限。玩家在面板上自己輸入金額,打到會超過就不再吃輸入 |
...
| `weightEdgeHold` | `0` | 【贏】衝到邊緣半截懸空,搖晃兩下後穩住。預設 0 = 關閉 |
| `weightSplash` | `1` | 【輸】直接砸進海裡 |
| `weightEdgeTip` | `0` | 【輸】觸艦、搖晃兩下,最後前傾翻落海中。預設 0 = 關閉 |
